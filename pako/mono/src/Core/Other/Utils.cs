...
namespace Core.Other
{
    ...
    static public class Utils
    {
        ...
        static object obj6 = new object();
        static List<string> vars; 
        static string m_osversion;
        ...
        static public SessionHandler Sh
        {
            get { lock (obj4) { return session; } }
            set { lock (obj4) { session = value; } }
        }
        static public List<string> Variables
        {
            get { lock (obj5) { return vars; } }
            set { lock (obj5) { vars = value; } }
        }
        ...
        /// <returns></returns>
        static public string FormatEnvironmentVariables(string Source, Response r)
        {
            if (Source == null)
                return null;
            ...
            string res = Source
                         .Replace("{" + strs[5] + "}", r.Language)
                         .Replace("{" + strs[6] + "}", r.Access.ToString())
                         .Replace("{" + strs[1] + "}", r.Connection.Server)
                         .Replace("{" + strs[3] + "}", r.MUser != null ? r.MUser.Jid.Bare : r.Msg.From.Bare)
                         .Replace("{" + strs[7] + "}", DateTime.Now.Date.ToString("d"))
                         .Replace("{" + strs[8] + "}", DateTime.Now.ToString("HH:mm:ss"))
                         .Replace("{" + strs[9] + "}", DateTime.Now.ToString("dd"))
                         .Replace("{" + strs[10] + "}", DateTime.Now.ToString("mm"))
                         .Replace("{" + strs[11] + "}", DateTime.Now.ToString("ss"))
                         .Replace("{" + strs[12] + "}", DateTime.Now.ToString("yyyy"))
                         .Replace("{" + strs[13] + "}", DateTime.Now.ToString("MM"))
            ...
            if (r.MUC != null && r.MUser != null)
                res = res.Replace("{" + strs[0] + "}", r.MUser)
                         ...
                         .Replace("{" + strs[4] + "}", r.MUser.Status)
                         .Replace("{" + strs[16] + "}", r.MUC.MyNick);
       ...
       {
        get { lock (obj6) {return m_osversion; }}
        set {lock (obj6) { m_osversion = value; }}
        }
       ...
       static public string GetOSVersion(Session S)
       {
               ...
               switch (Utils.OS)
               {
                   case Platform.Windows:
                       return std.Execute("ver", S);
                   case Platform.Unix:
                       return std.Execute("uname -a", S);
                   case Platform.Linux:
                       return std.Execute("uname -a", S);
                   default:
                       return "<unknown>";
       ...
       static public Dictionary<string, string>  Bot
       {
           get
           {
               Dictionary<string, string> dict = new Dictionary<string, string>();
               dict.Add("os", OSVersion);
