...
namespace Core.Kernel
{
    public class VCardCB
    {
        ...
        public VCardCB(MUser user, MUC muc, SessionHandler sh, string language, XmppClientConnection conn)
        {
            //m_r = new Response(conn, "", lng, 100000);
			m_r = new Response(sh.S.Rg[sh.S.Config.Language]);
            m_r.Access = 0;
            ///r.Msg = m_msg;
            m_r.MSGLimit = sh.S.Config.MucMSGLimit;
            m_r.MUC = muc;
            m_r.MUser = user;            
            m_r.Sh = sh;
        ...
        private void LastExtractor(object obj, IQ iq, object arg)
        {
            ...
            if (iq.Vcard != null)
            {
                ...
                else
                {
					...
					_emptyVcard = answer == "\n";
                    answer = answer == "\n" ? _mUser.Nick + " - "  + m_r.f("vcard_empty") : _mUser.Nick + " - "  + m_r.f("vcard_info")+answer.TrimEnd('\n');
                }
            }
            else
            {
                answer = _mUser.Nick + " - "  + m_r.f("version_error", jid);
				_emptyVcard = true;
            }

			if (_muc.OptionsHandler.GetOption("show_newuser_vcard") == "+" && _mUser.Affiliation == Affiliation.none)
			{
	            //m_r.Reply(answer);
				Message msg = new Message();
                msg.To = m_r.MUC.Jid;
                msg.Type = MessageType.groupchat;
                msg.Body = answer;//Utils.FormatEnvironmentVariables(ws[2], m_r);
