	...
	public class WwwHandler
	{
		...
		private bool check_content (string content_type)
		{
			...
			foreach (string s in r)
				if (content_type.IndexOf (s.ToLower ()) > -1)
					return true;
			return false;
		}
					...
					if (ws.Length > 2) {
						try {
							...
							foreach (string head in wp.Headers.Keys) {
								if (head.ToLower ().IndexOf ("cookie") > -1)
									continue;
								data += "\n" + head;
								for (int i = 0; i < 30 - head.Length; i++)
									data += " ";
					...
					if (ws.Length > 2) {
						try {
							...
							if (reply.Status == IPStatus.Success) {
								pattern = pattern.Replace ("{1}", reply.Address.ToString ()).Replace ("{2}", reply.RoundtripTime.ToString ()).Replace ("{3}", reply.Options.Ttl.ToString ()).Replace ("{4}", reply.Buffer.Length.ToString ());
								m_r.Reply (pattern);
							} else {
