...
namespace Plugin
{
    public class PingCB
    {
        ...
        private void VersionExtractor(object obj, IQ iq, object arg)
        {
            ...
            string data =
                (ts.Days > 31 ? Convert.ToString(System.Math.Truncate((double)ts.Days / 31)) + m_r.f("month") + " " : "") +
                (ts.Days > 0 ? ts.Days.ToString() + " " + m_r.f("day") + " " : "") +
                (ts.Hours > 0 ? ts.Hours.ToString() + " " + m_r.f("hour") + " " : "") +
                (ts.Minutes > 0 ? ts.Minutes.ToString() + " " + m_r.f("minute") + " " : "");
