...
namespace Core.Client
{
   ...
   /// <param name="reconnects"></param>
       public Session(string LanguageFolder, string PluginsFolder, string ConfigFile, string AutoMucsFile, string CensorFile, string UsersDataFile, string AccessFile,string ErrorFile, string TempDBFile, string botversion, int reconnects,  SessionHandler sh)
       {
           ...
           m_resphnd = new RManager(m_con, dir_langs, S.Config.MSGLimit);
           ...
           {
               CMDHandler cmdh = new CMDHandler(msg, Sh);
           };
       ...
       {
           S.C.SendMyPresence();
           foreach (AutoMuc am in S.AMManager.GetAMList())
           {
       ...
       public int GetAccess(Message msg, MUser user)
       {
           ...
           UserData ud = S.UDManager.GetUserData(Jid);
           if (ud != null)
           {
               if (ud.Access != null)
           ...
           switch (pres.Type)
           {
               ...
               case PresenceType.error:
                   if (m_muc != null)
                   {
                       ...
                       {
                           S.DelMUC(new Jid(pres.From.Bare));
                       }
                   }
                   break;
               case PresenceType.available:
                   ...
                   UserData ud = Sh.S.UDManager.GetUserData(Jid);
                   ...
                   if (pres.MucUser != null)
                   {
                       ...
                       string time;
                       if (m_muc != null)
                           Console.WriteLine(m_user != null ? "[" + p_jid.User + "]*** " + p_jid.Resource + " is now " + pres.Show.ToString().Replace("NONE", "Online") : "[" + p_jid.User + "]*** " + p_jid.Resource + " enters the room as " + pres.MucUser.Item.Affiliation + "/" + pres.MucUser.Item.Role);
                       ...
                       if (m_muc != null)
                       {
                           Response r = new Response(Sh.S.RManager.GetResponse(lng));
                           if (pres.Status != null)
                           {
                               string censored = S.GetMUC(p_jid).IsCensored(pres.Status);
                               if (censored != null)
                               {
                                   Console.WriteLine(m_muc.KickableForCensored(user).ToString());
               ...
               case PresenceType.unavailable:
                   if (pres.MucUser != null)
                   {
                       ...
                       if (m_user != null)
                       {
                           ...
                           Console.WriteLine("[" + p_jid.User + "]*** " + p_jid.Resource + " leave the room");
                       }
                       else
                           return;
                       if (p_jid.Resource == m_muc.MyNick)
       ...
       */
       ...
       public bool DelMUC(Jid Jid)
       {
           lock (sobjs[1])
           {
               foreach (MUC muc in S.MUCs.Values)
               {
                   if (muc.Jid.ToString() == Jid.ToString())
                   {
                       mucs_count--;
