...
namespace Core.Client
{
    ...
    public class CMDHandler
    {
        ...
        public CMDHandler(agsXMPP.protocol.client.Message msg, SessionHandler s)
        {
            m_msg = msg;
        ...
        public void _Handle()
        {
            ...
            if ((m_type == msgType.MUC) || (m_type == msgType.Roster))
            {
                ...
                UserData ud = Sh.S.UDManager.GetUserData(m_jid);
                bool ud_lang = ud != null;
                if (ud_lang)
                    ud_lang = ud.Language != null;
                r = new Response(Sh.S.RManager.
                    GetResponse(
                                 ud_lang ?
                                 ud.Language :
                                 is_muser ?
                ...
                if (is_muser)
                {
                    Console.WriteLine("[" + s_jid.User + "] " + s_jid.Resource + "> " + m_msg.Body);
                    ...
                    naccess = Sh.S.ACSManager.GetAccess(m_body);
                ...
                Console.WriteLine(m_body);
                   ...
                   if ((Sh.S.PHandler.Handles(m_cmd)) || (alias_exists))
                   {
                ...
                if (Sh.S.PHandler.Handles(m_cmd))
                {
                    ...
                    try
                    {
                        object obj = Sh.S.PHandler.Execute(m_cmd);
                        if (obj != null)
                        {
                    ...
                    case "help":
                        {
                            ...
                            if (args.Length > 1)
                            {
                                ...
                                m_retort = r.FormatPattern("help_info", Sh.S.PHandler.GetPluginsList(), d + "<volume> <command>");
                            break;
                        }
