...
namespace Core.Conference
{
    ...
    public class MUC
    {
        ...
        /// <returns></returns>
        public string GetAlias(string phrase, ref string from, ref string val, ref string to, MUser user)
        {
            lock (sobjs[22])
            {
                ...
                foreach (Element el in Aliases.RootElement.SelectElements("alias"))
                {
                    ...
                    value = Utils.FormatEnvironmentVariables(value, this, user);
                    val = value;
                    from = name;
        ...
        public bool chkal_rec(string alias_name, int curr_level, int max_level, MUser user)
        {
            ...
            {
                if (curr_level >= max_level) return true;
                alias_name = GetAlias(alias_name, ref b, ref c, ref a,user); curr_level++;
            }
            @out.exe(curr_level.ToString() + "  " + alias_name);
            ...
            if (ws.Length > 2) if (ws[0].ToLower() == "misc" && ws[1] == "make")
                {
                    ...
                    curr_level += ps.Length - 1; foreach (string p in ps)
                        if (chkal_rec(p, curr_level, max_level, user)) 
                            return true;
                        return false;
                }
            return false;
        ...
        public MUC(XmppClientConnection con, Jid Jid, string Nick, string Status, string Lang, ShowType Show, SessionHandler sh, string Password)
        {
            ...
            m_defs = new Dictionary(Utils.GetPath("dict", this), sqlv);
