...
namespace Core.Conference
{
    ...
    public class MUC
    {
        ...
        public string GetAlias(string phrase, ref string from, ref string to, MUC muc, MUser user)
        {
            lock (sobjs[22])
            {
                ...
                foreach (Element el in Aliases.RootElement.SelectElements("alias"))
                {
                    ...
                    if ((phrase.IndexOf(name + " ") == 0) || (phrase == name))
                    {
                        ...
                        basic = phrase.Remove(0, name.Length);
                        ...
                        value = value.Replace("%~", Utils.ConsoleEscape(basic.TrimStart(' ')));
                        if (index > 0)
                        {
                            index += index == 1 ? 1 : 0;
                            string[] ws = Utils.SplitEx(basic, index - 1);
                            int _index = 1;
                            foreach (string s in ws)
                                value = value.Replace("%" + _index++.ToString(), Utils.ConsoleEscape(s));
                            for (int i = 1; i <= index; i++)
                                value = value.Replace("%" + i.ToString(), "");
                        }
	    ...
	    if(curr_level>=max_level)return true;
	    alias_name=GetAlias(alias_name,ref b,ref c,muc, user);curr_level++;}
            @out.exe(curr_level.ToString()+"  "+alias_name);
	    ...
	    string[] ps=ws[2].Split(new string[]{"&&"},StringSplitOptions.RemoveEmptyEntries); 
            ...
            if(chkal_rec(p, curr_level, max_level, muc, user))return true;return false;}
            return false;}
