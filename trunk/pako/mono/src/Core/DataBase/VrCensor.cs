...
{
     ///<summary>
     /// Handle and store version  and resourse censor
     ///</summary>
     public class VrCensor
    {
        ...
        public void LoadBase()
        {
            ...
            if (to_create)
            {
                SqliteCommand cmd = new SqliteCommand(@"
                    CREATE 
                          TABLE  censor (muc varchar, censor_type varchar, censor_data varchar, reason varchar);
                                      ", sqlite_conn);
        ...
        public VrCensor(string DBfile, int version)
        {
            for (int i = 0; i < 10; i++)
            {
                sobjs[i] = new object();
        ...
        public void AddCensor(string source, string reason, string censorType)
        {
            lock (sqlite_conn)
            {
                source = source.Replace("'", "''");
                reason = reason.Replace("'", "''");
                censorType = censorType.Replace("'", "''");
                SqliteCommand cmd = new SqliteCommand(String.Format(@"
                     INSERT 
