...
namespace Core.Kernel
{
   ...
   /// <param name="reconnects"></param>
       public Session(string LanguageFolder, string PluginsFolder, string ConfigFile, string AutoMucsFile, string CensorFile, string VipAccessFile, string VipLangFile, string AccessFile, string ErrorFile, string TempDBFile, string botversion, int reconnects, SessionHandler sh)
       {
           for (int i = 0; i < 60; i++)
           {
               sobjs[i] = new object();
           ...
           @out.write("DebugMode " + (Sh.arg_debug ? "ON" : "OFF"));
           ...
           m_va = new VipAccess(dir_va);

           m_vl = new VipLang(dir_vl);
           m_censor = new Censor(dir_censor);
           ...
           m_con.AutoResolveConnectServer = false;
           ...
           m_con.OnClose += delegate(object o)
           {
               ...
               {
                   @out.write("Reconnect in " + S.Config.ReconnectTime + " seconds (" + m_rects + "/" + S.Config.MaxReconnects + ")\n");
       ...
       public int? GetCMDAccess(string cmd, Response r, bool muc)
       {
           if (muc && r.MUC != null && r.MUser != null)
           {
               int? access = r.MUC.OptionsHandler.GetOption("cmdaccess") == "+" ? r.MUC.AccessManager.GetAccess(cmd) : null;

               if (access == null)
               {
                   string a = "", b = "";
                   cmd = r.MUC.GetAlias(cmd, ref a, ref b, r.MUC, r.MUser);
                   access = AccessManager.GetAccess(cmd);
               }
               access = Utils.SplitEx(cmd, 2)[0].ToLower() == "admin" && access == null ? 100 : access;
               return access;
           }
           else
           {

               int? access_ = AccessManager.GetAccess(cmd);
               return Utils.SplitEx(cmd, 2)[0].ToLower() == "admin" && access_ == null ? 100 : access_;
       ...
       public Jid GetMUCJid(string RealIndexString)
       {
           try
           {
               ...
               return (index <= Sh.S.MUCs.Keys.Count && --index > -1) ?
                   (Jid)new ArrayList(Sh.S.MUCs.Keys)[index] : null;
           }
           catch { return null; } 
       }
