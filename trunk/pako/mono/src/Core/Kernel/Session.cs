...
namespace Core.Kernel
{
   ...
   /// <param name="reconnects"></param>
       public Session(string LanguageFolder, string PluginsFolder, string ConfigFile, string AutoMucsFile, string CensorFile, string VipAccessFile, string VipLangFile, string AccessFile, string ErrorFile, string TempDBFile, string botversion, int reconnects, SessionHandler sh)
       {
           ...
           m_acsh = new AcessManager(dir_access);
           ...
           {
               CommandHandler cmdh = new CommandHandler(msg, Sh, null);
           };
           ...
           {
               PresenceHandler pr_handler = new PresenceHandler();
               pr_handler.Handle(pres, Sh);
           };
       ...
       public bool Self(MUC muc, MUser user, string entity)
       {
       ...
       public bool JidValid(Jid Jid)
       {
           return Jid.ToString().IndexOf("@") > -1;
       }

       public bool JidValid(string Jid)
       {
       ...
       public MUC GetMUC(Jid Jid)
       {
           lock (sobjs[1])
           {
               return (MUC)S.MUCs[new Jid(Jid.Bare)];
           }
       }
