    ...
    public class ConfigHandler
    {
        ...
        public void Handle()
        {
            ...
            switch (cmd)
            {
                case "list":
                    {
                        rs = m_r.f("volume_list", n) + "\nlist, jid, nick, status, censor, set_help, set_pattern, get_pattern, pl_load, pl_info, pl_unload, proc_show, proc_kill, proc_new, muc_lang, muc_status, muc_nick, cmdaccess, dict_clear, vipaccess, viplang, gmsg, cmd, quit, restart, find, langs, eval, password, prefix, usessl, compression, debug, autosub, starttls, myroot, port, rectime, reconnects, msglimit, lang, errors, pako, heap";
                        break;
                    }
                ...
                case "emul":
                    {
                        //*admin emul roso@conference.jabber.ru *muc echo gg!!!
                        ws = Utils.SplitEx(m_b, 3);
                        if (ws.Length > 3)
                        {
                            Jid muc = new Jid(ws[2]);
