...
namespace Plugin
{
	...
	public class MucHandler
	{
		...
		public void Handle ()
		{
			...
			case "vrcensor":
				// TODO: Version and resource censor manager
			...
			case "show":
				
				{
					...
					if (ws.Length < 4) {
						bool _shortOutput = false;
						if (ws.Length > 2) {
							_shortOutput = ws[2].Trim () == "short";
						}
			...
			case "info":
				
				{
					...
					if (m_user != null) {
						...
						string lng = vl != null ? vl : m_user.Language;
						...
						if (m_r.Msg.Type == MessageType.groupchat)
							ShowJid = "???"; else if (m_r.Access <= 50)
							ShowJid = "???";
						
						int? access = Sh.S.GetAccess (msg, m_user, m_r.MUC);
						if ((m_r.Emulation != null) && (ws.Length == 2))
							access = 100;
						rs = m_r.f ("muc_user_info", m_user, m_user.Affiliation + "/" + m_user.Role, lng, (access ?? 0).ToString (), m_user.Show.ToString ().Replace ("NONE", "Online") + " (" + m_user.Status + ")", new DateTime (m_user.EnterTime).ToString (), ShowJid, Utils.FormatTimeSpan ((DateTime.Now.Ticks - m_user.Idle), m_r));
					}
					break;
				}
			...
			if (syntax_error)
				...
				m_r.se (self); else if (rs != null && rs != "")
				m_r.Reply (rs);
