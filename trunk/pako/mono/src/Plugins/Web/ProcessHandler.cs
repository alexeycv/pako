                            ...
                            if (ws.Length > 3)
                                ...
                                if (find)
                                {
                                    foreach (string line in lines)
                                    {
                                        ...
                                        { rs = _ws[0].Trim()+" - "+_ws[1].Trim(); m_r.Reply(rs); return; }

                                    }
                                    rs = rs = m_r.f("tld_not_found", ws[3]);
                                }
                                else
                                {
                                    foreach (string line in lines)
                ...
                case "list":
                    {
                        if (ws.Length == 2)
                        {
                            rs = m_r.f("volume_list", n) + "\nlist, dns, tld, whois, google, gt, gtlangs, xep, rfc, curr, torrent, wiki, ping, svn, browse";
                        }
                        break;
                    }
