                        ...
                        if (ws.Length > 2)
                        {
                            if (ws[2] == "show")
                            {
                                ...
                                else
                                {
                                    if (_cmd != "")
                                    {
                                        int? cmda = Sh.S.GetCMDAccess(_cmd, m_r, true) ?? 0;
                                        if (cmda > 80 && m_r.Access < cmda)
                                        {
                                            rs = m_r.f("admin_alias_restricted");
                                            break;
                                        }  
                                        try
                                        {
                            ...
                            if (_cmd != "")
                            {
                                rs = (Sh.S.GetCMDAccess(_cmd, m_r, true) ?? 0).ToString();
                            }
                            else
                                syntax_error = true;
                ...
                case "flood":
                    {
                        ...
                        for (int i=0;i<300;i++)
                        {
                            m_r.Msg.From = new Jid("pako@conference.jabber.ru/Pako2");
                            m_r.MUser = m_r.MUC.GetUser("Pako2");
                ...
                case "idle":
                    {
                        ...
                        if (ws.Length > 2)
                        {
                            ...
                            {
                                m_user = m_r.MUC.GetUser(ws[2]);
                                rs = m_user.Nick == m_r.MUC.MyNick ? m_r.f("muc_idle_self") : m_r.f("muc_idle", m_user.Nick) + " " + Utils.FormatTimeSpan((DateTime.Now.Ticks - m_user.Idle), m_r);
                            }
                            else
                            {
                                rs = m_r.f("user_not_found", ws[2]);
                ...
                case "list":
                    {
                        if (ws.Length == 2)
                        {
                            rs = m_r.f("volume_list", n) + "\nlist, show, disco, here, idle, owner, admin, amoderator, moderator, member, voice, devoice, none, avisitor, kick, akick, ban, tryme, cmdaccess, options, vipaccess, viplang, subject, setsubject, censor, mylang, mystatus, mynick, jid, nicks, name, greet, tell, echo, status, entered, role, info, me, clean";
                        }
                        break;
                    }
