    ...
    public class ConfigHandler
    {
        ...
        public void Handle()
        {
            ...
            switch (cmd)
            {
                case "list":
                    {
                        rs = m_r.f("volume_list", n) + "\nlist, emul, censor, config, set_help, set_pattern, get_pattern, pl_load, pl_info, pl_unload, proc_show, proc_kill, proc_new, muc_lang, muc_status, muc_nick, cmdaccess, dict_clear, vipaccess, viplang, gmsg, cmd, quit, restart, find, langs, eval, myroot, errors, pako, heap";
                        break;
                    }
                        ...
                        if (ws.Length > 2)
                        {
                            if (ws[2] == "show")
                            {
                                ...
                                if (ws[2] == "del")
                                {
                                    ...
                                    if (ws.Length == 3 && _cmd != "")
                                    {
                                        Sh.S.AccessManager.DelCommand(_cmd);
                                        rs = m_r.Agree();
                                    }
                                    else
                                        syntax_error = true;
                            ...
                            if (_cmd != "")
                            {
                                ...
                                rs = (Sh.S.GetCMDAccess(_cmd, m_r, false) ?? 0).ToString();
                            }
                            else
                                syntax_error = true;
