...
namespace Core.Manager
{
    ...
    public class VipManager: XMLContainer
    {
        Vipuser[] m_users;
        int m_count;


        public VipManager(string UsersFile)
        {
            Open(UsersFile, 3);
            GetUDList();
        ...
        public void GetUDList()
        {
            ...
            lock (this)
            {
                ...
                m_users = new Vipuser[m_count];

                int ii = 0;

                for (int i = 0; i < m_count; i++)
                {
                    m_users[i] = new Vipuser(null, null, null);
                }
                ...
                foreach (Element el in Document.RootElement.SelectSingleElement("usersdata").SelectElements("user"))
                {
                    ...
                    m_users[ii] = new Vipuser(new Jid(el.GetAttribute("jid")), m_l, m_a);
                    ii++;
                }
            }
        }
        ...
        /// <returns></returns>
        public Vipuser GetUserData(Jid Jid)
        {
            lock (UsersData)
            {
                if (Jid != null)
                {
                    ...
                    foreach (Vipuser ud in UsersData)
                    {
