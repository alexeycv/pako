            ...
            switch (cmd)
            {
                ...
                case "kick":
                    {
                        if (ws.Length > 2)
                        {
                           ...
                           if (reason == null)
                               reason = m_r.FormatPattern("kick_reason");
                            ws = Utils.SplitEx(m_b, 2);
                            switch (m_r.MUC.Kick(ws[2], reason))
                            {
				...
				MUser user = m_r.MUC.GetUser(ws[2]);
                                   ...
                                   for (int i = 2; i < ws.Length - 1; i++)
					{
                                   if (ws[i].StartsWith("@"))
				      {
					switch (ws[2].ToLower().Substring(1))
					{
					   case "jid-exp":
					   ...
					   case "nick-exp":
						...
						break;
					   default:
					        m_r.Reply(m_r.FormatPattern("akick_wrong_parameter"));
						return;
					}
					  ...
					  if (!reg.IsMatch((ws[i].Substring(1))
						{
                                                  m_r.Reply(m_r.FormatPattern("akick_wrong_parameter"));
					          return;
 						}
                                          foreach (Match m in reg.Matches(ws[i].Substring(1))
						{
							...
							{
								case 's':
								     ticks += Convert.ToInt32(time_span)*10000000;
								     break;
								case 'm':
								     ticks += Convert.ToInt32(time_span)*600000000;
								     break;
								case 'h':
								     ticks += Convert.ToInt32(time_span)*36000000000;
								     break;
								case 'd':
								     ticks += Convert.ToInt32(time_span)*864000000000;
								     break;
								case 'M':
 								     ticks += Convert.ToInt32(time_span)*25920000000000;
								     break;
								default:
								     m_r.Reply(m_r.FormatPattern("akick_wrong_parameter"));
						                     return;
                            ...
                            // *muc akick @jid-exp $15h ^[0-9]+\@\s*$
                            // *muc akick @jid-exp      ^[0-9]+\@\s*$ 
                            // *muc akick $15h          ^[0-9]+\@\s*$
			    // *muc akick               ^[0-9]+\@\s*$	
                            switch (m_r.MUC.Kick(ws[2], reason))
                            {
                                    ...
                                    default:
                                        {
                                            ...
                                            if (reason == null)
                                                reason = m_r.FormatPattern("tryme_reason");
                                            ws = Utils.SplitEx(m_b, 2);
                ...
                case "tell":
                    {
                        ...
                        if ((ws.Length > 2) && (word != ""))
                        {
                            ...
                            if (Sh.S.Tempdb.AddTell(tell_jid, Utils.FormatEnvironmentVariables(ws[2], m_r.MUC, m_r.MUser), sender))
                                ...
                                rs = m_r.FormatPattern("tell_count_overflow");
                ...
                case "ban":
                    {
                        if (myaccess >= 70)
                        {
                            if (ws.Length > 2)
                            {
                                ...
                                if (reason == null)
                                    reason = m_r.FormatPattern("ban_reason");
                                ws = Utils.SplitEx(m_b, 2);
                ...
                case "admin":
                    {
                        if (myaccess >= 80)
                        {
                            if (ws.Length > 2)
                            {
                                ...
                                if (reason == null)
                                    reason = m_r.FormatPattern("admin_reason");
                                ws = Utils.SplitEx(m_b, 2);
                ...
                case "owner":
                    {
                        if (myaccess >= 80)
                        {
                            if (ws.Length > 2)
                            {
                                ...
                                if (reason == null)
                                    reason = m_r.FormatPattern("owner_reason");
                                ws = Utils.SplitEx(m_b, 2);
                        ...
                        if (myaccess >= 50)
                        {
                            if (ws.Length > 2)
                            {
                                if (m_r.MUC.UserExists(ws[2]))
                                {
                                    ...
                                    if (reason == null)
                                        reason = m_r.FormatPattern("voice_reason");
                                    ws = Utils.SplitEx(m_b, 2);
                        ...
                        if (myaccess >= 50)
                        {
                            if (ws.Length > 2)
                            {
                                if (m_r.MUC.UserExists(ws[2]))
                                {
                                    ...
                                    if (reason == null)
                                        reason = m_r.FormatPattern("devoice_reason");
                                    ws = Utils.SplitEx(m_b, 2);
