...
namespace www
{
    ...
    public class Google
    {
        ...
        void Search()
        {
            ...
            string _res_ = res;
             FileStream fs = File.Open("gg.txt", FileMode.Append);
              StreamWriter  ssr = new StreamWriter(fs);
                ssr.Write(res);
                ssr.Close();
                fs.Close();
            ...
            while (regex.IsMatch(res) && (m_count > count))
            {
                ...
                success = true;
             //   Regex _regex = new Regex("</b> of about <b>(.*)</b>");
               all = Utils.GetValue(_res_, "</b> of about <b>(.*)</b> for <b>");
               if ((all == "") || (all.Length > 30))
                all = Utils.GetValue(_res_, "</b> of <b>(.*)</b> for <b>");
              /* MatchCollection mc  = _regex.Matches(res);
               if (mc.Count > 0)
                  all = Utils.GetValue(mc[0].ToString(), "</b> of about <b>(.*)</b>");
               else
                   { 
                     _regex = new Regex("</b> of <b>(.*)</b>");
                     mc  = _regex.Matches(res);
                     if (mc.Count > 0)
                     all = Utils.GetValue(mc[0].ToString(), "</b> of <b>(.*)</b>");
                     else
              ...
              while ((_res.IndexOf("<") != -1) && (_res.IndexOf(">") != -1))
                ...
                else
                {
                 ...
                 Console.WriteLine("--selecting done for a result");
                res = Utils.RemoveValue(res, "class=g><h2 class=r>(.*)</td></tr></table></div>", true);
                 Console.WriteLine("--previous selecting removed");
                count++;
            }
            if (!success)
                m_r.Reply(m_r.FormatPattern("google_nothing_found"));
